...
(function() {
    ...
    function processColumns() {
        ...
        document.querySelectorAll('div.col').forEach(col => {
            ...
            const blocks = col.innerHTML.split(/(?=\d{2}\/\d{2}\/\d{4}:)/);
        ...
        rows.forEach(row => {
            ...
            const dayBlocks = menuSectionRaw.split(/(?=\d{2}\/\d{2}\/\d{4}:)/).filter(b => b.trim() !== "");
            let cardsHtml = "";
    ...
    function openPickupManager() {
        ...
        routeOrder.forEach(name => {
            if (clients.includes(name)) {
                // Base client: checkbox to mark pickup
                const isChecked = data.normal.includes(name) ? 'checked' : '';
                listHtml += `<div style="margin-bottom:8px;"><label><input type="checkbox" class="tm-pickup-check" value="${name}" ${isChecked}> ${name}</label></div>`;
            } else {
        ...
        routeOrder.forEach(name => listHtml += `<option value="${name}">${name}</option>`);
